...
def equatorial__horizontal(observer_latitude, declination, right_ascension=None,
    ...
    latitude, hour_angle, declination = np.radians(
            [latitude, hour_angle, declination]
            )
...
def horizontal__equatorial(observer_latitude, altitude, azimuth):
    ...
    _num, _den = (
        np.cos(zenith_angle) - np.sin(latitude) * np.sin(declination),
...
def equatorial__ecliptic(right_ascension, declination):
    ...
    ec_latitude = np.arcsin(
            np.sin(dec) * np.cos(_ecliptic) -
            np.cos(dec) * np.sin(_ecliptic) * np.sin(ra)
            )

    _num, _den = (
        np.sin(ra) * np.cos(_ecliptic) + np.tan(dec) * np.sin(_ecliptic),
        np.cos(ra)
        )
...
def ecliptic__equatorial(ecliptic_latitude, ecliptic_longitude):
    ...
    dec = np.arcsin(
            np.sin(ec_latitude) * np.cos(_ecliptic) +
            np.cos(ec_latitude) * np.sin(_ecliptic) * np.sin(ec_longitude)
            )

    _num, _den = (
        np.cos(ec_latitude) * np.sin(ec_longitude) * np.cos(_ecliptic) -
...
def equatorial__galactic(right_ascension, declination):
    ...
    gal_long = np.arctan2(
            np.cos(dec) * np.sin(ra - _ra_gal),
            np.sin(dec) * np.cos(_dec_gal) -
            np.cos(dec) * np.sin(_dec_gal) * np.cos(ra - _ra_gal)
            ) - _long_ncp

    gal_lat = np.arcsin(
            np.sin(dec) * np.sin(_dec_gal) +
            np.cos(dec) * np.cos(_dec_gal) * np.cos(ra - _ra_gal)
            )
...
def galactic__equatorial(galactic_latitude, galactic_longitude):
    ...
    dec = np.arcsin(
            np.sin(gal_lat) * np.sin(_dec_gal) +
            np.cos(gal_lat) * np.cos(_dec_gal) * np.cos(_long_ncp - gal_long)
            )

    ra = np.arctan2(
            np.cos(gal_lat) * np.sin(_long_ncp - gal_long),
            np.sin(gal_lat) * np.cos(_dec_gal) -
            np.cos(gal_lat) * np.sin(_dec_gal) * np.cos(_long_ncp - gal_long)
            ) + _ra_gal
